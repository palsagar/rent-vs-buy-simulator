...
**Property levy**:
The recurring annual charge for occupying a home, whichever unit a region collects it in — ad-valorem (`property_tax_rate`, US/NL) or a flat cost-indexed amount (`annual_property_levy`, FR/DE/UK). One economic quantity in two representations, so a region shows whichever its bundle uses and never both. "Levy" is the noun on every surface — sliders, tornado bars, the cost breakdown — because the flat forms are not taxes on property: UK council tax is an occupier charge for local services, and calling it a property tax invites the reader to expect it to scale with the home's value.
_Avoid_: property tax, council tax (as a generic), millage
